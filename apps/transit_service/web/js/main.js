  ...
  try {
    api.onmessage = function(msg, data)  {
      if ("event" in data) {
        ...
        if (data.event == "NotifyViewOfDrone") {
          notify(data.details.message);
        }
        if (data.event == "UpdateBatteryPercent") {
          document.getElementById("battery-percent").innerHTML = data.details.percent.toFixed(2);
        }
      }
    }
  }
...
function notify(message) {
  var notificationsBar = document.getElementById("notification-bar"); 
  notificationsBar.innerHTML += message;
}
